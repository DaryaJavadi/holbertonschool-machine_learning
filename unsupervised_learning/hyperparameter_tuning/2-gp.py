#!/usr/bin/env python3
""" Task 2: 2. Update Gaussian Process """
import numpy as np
...
class GaussianProcess():
    ...
    def update(self, X_new, Y_new):
        """
        Updates the Gaussian Process with new sample data.

        Parameters:
        X_new (np.ndarray):
        New sample input(s) to update the model,shape (1,) or (n, 1).

        Y_new (np.ndarray):
        New sample output(s) corresponding to X_new, shape (1,) or (n, 1).

        This method appends the new data points to the existing samples
        and recomputes the covariance kernel matrix.
        """
